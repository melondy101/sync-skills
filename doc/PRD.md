...
**实现状态**：上述六阶段流程已于 2026-08-11 在分支 `codex/ui_optimization` 完整实现，并于 2026-08-17 随 PR #3 合入 main。卡片网格 + InstallDialog 单步安装 + 二次确认 + 市场源管理弹窗等实际形态与 §5.10 描述略有差异，详见：
- 设计文档：`docs/market-ui-redesign.md`（§4 重设计方案）
- 执行手册：`docs/market-ui-impl-guide.md`（任务 1–8 的提交级落地点）

PRD 之前累积的「待确定事项 1–98」列表已废弃——那些问题在市场 UI 实现中已逐项决策并落地，不再作为待办追踪。如对某一决策有疑问，按上述设计/实现文档中的对应章节查阅即可。
...
| 2026-08-18 | v1.2 | §5.10 移除「待确定事项 1–98」清单（市场 UI 实施后已全部决策并落地，见 `docs/market-ui-redesign.md` §4 / `docs/market-ui-impl-guide.md`），改为指向已实现设计文档 |
