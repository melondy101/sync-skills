...
- `skill-manager-mcp` 进入安装包：它就是 `src-tauri` 这个 crate 的第二个二进制（`src/bin/skill-manager-mcp.rs`），Tauri 打包时与主程序装进同一目录、同名，`mcp_config::default_entry()` 那条"取主程序同级目录下的 server"的规则因此在开发/安装两种形态下都成立，设置面板登记的命令路径真实可达
...
- 把自家第二个二进制声明成 `bundle.externalBin` 造成的两处构建失败（v0.3.0 出包过程中实测到，最终修法是把这条配置删掉）：
  - **全新 clone 构建不了**：`bundle.externalBin` 由 tauri 的构建脚本在**每一次** cargo 调用时校验 `src-tauri/bin/skill-manager-mcp-<triple>[.exe]`，而该文件只能由构建产物 stage 出来，于是 `cargo build` / `tauri dev` 一律报 `resource path ... doesn't exist`。中途曾用 `scripts/stage-sidecar.mjs --allow-placeholder`（无产物时写空占位打破环）绕过，随该配置一起删除；现在全新 clone 装完依赖直接 `pnpm tauri dev`。
  - **Windows MSI 出不了包**：同一个二进制既被 externalBin stage 一份、又被 cargo 的 `[[bin]]` 产物带一份，MSI 里就有两个组件装 `skill-manager-mcp.exe`，WiX `light.exe` 以 `ICE30`（"installed by two different components"）失败；NSIS 与 deb 没有这项校验，所以只有 Windows 的 `.msi` 炸。删掉 `externalBin` 与 `beforeBundleCommand` 后本机 `npx tauri bundle --bundles nsis,msi` 两个包都通过，且 `main.wxs` 里只剩一个 `Bin_skill_manager_mcp` 组件、Linux 的 `usr/bin/skill-manager-mcp` 照旧在
