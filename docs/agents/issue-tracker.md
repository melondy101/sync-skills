...
Issues and specs for this repo live as GitHub issues on `huang-yi-dae/sync-skills`. Use the `gh` CLI for all operations.
...
## Local working copy: `.scratch/<feature>/issues/`

The `.scratch/<feature>/issues/` folders (e.g. `.scratch/market-ux-polish/issues/`) are **local working copies** that mirror / draft the corresponding GitHub issues. They are **not** a separate tracker.

Rules:

- One folder per active effort (e.g. `market-ux-polish`, `phase-3-migration`).
...
Create a GitHub issue (or update the existing one referenced from the local draft).

## When a skill says "fetch the relevant ticket"

Run `gh issue view <number> --comments`. If the user hands you a `.scratch/<feature>/issues/NN-…` path, open that file and follow its `GitHub: #N` pointer to the canonical issue.
